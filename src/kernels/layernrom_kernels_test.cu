...
void printMatrix<half2>(half2* a, int m, int n){
    for(int i = 0;i < m;i ++){
        for(int j = 0;j < n;j ++){
            printf("%f %f ", __half2float(a[i * n + j].x), __half2float(a[i * n + j].y));
        }puts("");
    }
    puts("");
}
...
void layernorm_kernel_half2_test(){
    ...
    for(int i = 0;i < m;i ++){
        for(int j = 0;j < n;j ++){
            input[i * n + j] = half2(__float2half((float)(i * n + j * 2)), __float2half((float)(i * n + j * 2 + 1)));
    ...
    llm::kernel::invoke_layernorm_kernel<half2>(dout, dinput, dweight, dbias, epsilon, m, n);
    // llm::kernel::invoke_half2_layernorm_kernel(dout, dinput, dweight, dbias, epsilon, m, n);
...
void layernorm_kernel_float_test(){
    ...
    llm::kernel::invoke_layernorm_kernel<float>(dout, dinput, dweight, dbias, epsilon, m, n);
    // llm::kernel::invoke_float_layernorm_kernel(dout, dinput, dweight, dbias, epsilon, m, n);
