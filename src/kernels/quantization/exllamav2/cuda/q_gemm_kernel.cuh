...
__forceinline__ __device__ half2 dot22_8(half2 (&dq)[4],
                                         const half* a_ptr,
...
__forceinline__ __device__ half2 dot22_16(half2 (&dq)[8],
                                          const half* a_ptr,
                                          const half2 g_result,
                                          const half qs_h) {
  half2 result = {};
...
__forceinline__ __device__ half2 dot22_32(half2 (&dq)[16],
                                          const half* a_ptr,
                                          const half2 g_result,
                                          const half qs_h) {
  half2 result = {};
...
__forceinline__ __device__ float dot22_16_f(half2 (&dq)[8],
                                            const half* a_ptr,
                                            const float g_result,
                                            const float qs_f) {
  half2 result = {};
...
__forceinline__ __device__ float dot22_32_f(half2 (&dq)[16],
                                            const half* a_ptr,
                                            const float g_result,
                                            const float qs_f) {
  half2 result = {};
...
typedef void (*fp_gemm_half_q_half_kernel)(const half*,
                                           const uint32_t*,
                                           const uint32_t*,
                                           const half*,
                                           half*,
...
__global__ void gemm_half_q_half_kernel(const half* __restrict__ a,
...
#pragma unroll
    for (int j = 0; j < 4; j++) {
      ...
      for (int m = 0; m < m_count; m++) {
        block_c[m][0] =
            dot22_8_f(dq[0], a_ptr + m * a_stride, block_c[m][0], qs_f0);
        block_c[m][1] =
            dot22_8_f(dq[1], a_ptr + m * a_stride, block_c[m][1], qs_f1);
        block_c[m][2] =
            dot22_8_f(dq[2], a_ptr + m * a_stride, block_c[m][2], qs_f2);
        block_c[m][3] =
            dot22_8_f(dq[3], a_ptr + m * a_stride, block_c[m][3], qs_f3);
      }
      a_ptr += 8;
    }
    k += 32;
...
#pragma unroll
    for (int j = 0; j < 2; j++) {
      ...
      half2 dq[4][8];
      dequant_6bit_16(
          load_int4[0].x, load_int4[1].x, load_int4[2].x, dq[0], size_n);
      dequant_6bit_16(
          load_int4[0].y, load_int4[1].y, load_int4[2].y, dq[1], size_n);
      dequant_6bit_16(
          load_int4[0].z, load_int4[1].z, load_int4[2].z, dq[2], size_n);
      dequant_6bit_16(
          load_int4[0].w, load_int4[1].w, load_int4[2].w, dq[3], size_n);

      for (int m = 0; m < m_count; m++) {
        block_c[m][0] =
            dot22_16_f(dq[0], a_ptr + m * a_stride, block_c[m][0], qs_f0);
        block_c[m][1] =
            dot22_16_f(dq[1], a_ptr + m * a_stride, block_c[m][1], qs_f1);
        block_c[m][2] =
            dot22_16_f(dq[2], a_ptr + m * a_stride, block_c[m][2], qs_f2);
        block_c[m][3] =
            dot22_16_f(dq[3], a_ptr + m * a_stride, block_c[m][3], qs_f3);
      }
      a_ptr += 16;
    }
    k += 32;
...
#pragma unroll
    for (int j = 0; j < 1; j++) {
      ...
      for (int m = 0; m < m_count; m++) {
        block_c[m][0] =
            dot22_32_f(dq[0], a_ptr + m * a_stride, block_c[m][0], qs_f0);
        block_c[m][1] =
            dot22_32_f(dq[1], a_ptr + m * a_stride, block_c[m][1], qs_f1);
        block_c[m][2] =
            dot22_32_f(dq[2], a_ptr + m * a_stride, block_c[m][2], qs_f2);
        block_c[m][3] =
            dot22_32_f(dq[3], a_ptr + m * a_stride, block_c[m][3], qs_f3);
      }
      a_ptr += 32;
    }
...
fp_gemm_half_q_half_kernel pick_gemm_half_q_half_kernel(bool first_block,
                                                        const int m_count) {
