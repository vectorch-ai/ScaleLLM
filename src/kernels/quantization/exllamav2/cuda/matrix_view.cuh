...
class MatrixView_half {
 public:
  ...
  __device__ __forceinline__ MatrixView_half(const half* data,
                                             const int height,
                                             const int width)
  ...
  __device__ __forceinline__ void item4(half (&items)[4],
                                        int row,
                                        int column) const {
  ...
  __device__ __forceinline__ void item4_f(float (&items)[4],
                                          int row,
                                          int column) const {
    half2* ptr = (half2*)item_ptr(row, column);
    half2 i01 = ptr[0];
  ...
  __device__ __forceinline__ void item4_h2(half2 (&items)[4],
                                           int row,
                                           int column) const {
    half2* ptr = (half2*)item_ptr(row, column);
    half2 i01 = ptr[0];
...
class MatrixView_half_rw {
 public:
  ...
  __device__ __forceinline__ MatrixView_half_rw(half* data,
                                                const int height,
                                                const int width)
      : data(data), height(height), width(width) {}
  ...
  __device__ __forceinline__ void set4(int row,
                                       int column,
                                       half v0,
                                       half v1,
                                       half v2,
...
class MatrixView_q4_row {
 public:
  ...
  __device__ __forceinline__ void item2(int (&items)[2],
                                        int row,
                                        int column) const {
    int shift = (column & 0x07) * 4;
  ...
  __device__ __forceinline__ void item4(int (&items)[4],
                                        int row,
                                        int column) const {
    int shift = (column & 0x07) * 4;
    uint32_t d = data[row * width / 8 + column / 8] >> shift;
