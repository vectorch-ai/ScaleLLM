...
QMatrix::QMatrix(const int _device,
                 const int _height,
                 const int _width,
                 const int _groups,
  ...
  if (!is_gptq) {
    ...
    cudaMemcpy(cpu_q_groups,
               cuda_q_groups,
               groups * 2 * sizeof(uint16_t),
  ...
  shuffle_kernel<<<gridDim, blockDim, 0, at::cuda::getCurrentCUDAStream()>>>(
      cuda_q_weight,
      height,
      width,
      rows_8,
...
__global__ void reconstruct_gptq_kernel(
  ...
  while (k < end_k) {
    ...
    for (int p = 0; p < 4; p++) {
      ...
      dequant_4bit_8_gptq(
          load_int4.x, dq[0], z1z16[0], y1y16[0], size_n, false);
      dequant_4bit_8_gptq(
          load_int4.y, dq[1], z1z16[1], y1y16[1], size_n, false);
      dequant_4bit_8_gptq(
          load_int4.z, dq[2], z1z16[2], y1y16[2], size_n, false);
      dequant_4bit_8_gptq(
          load_int4.w, dq[3], z1z16[3], y1y16[3], size_n, false);
...
void QMatrix::reconstruct(half* out) {
  ...
  if (!is_gptq) {
    reconstruct_kernel<<<gridDim,
                         blockDim,
                         0,
                         at::cuda::getCurrentCUDAStream()>>>(cuda_q_weight,
                                                             cuda_q_perm,
                                                             cuda_q_scale,
                                                             cuda_q_scale_max,
                                                             // cuda_q_groups,
    ...
    reconstruct_gptq_kernel<<<gridDim,
                              blockDim,
                              0,
...
__global__ void make_sequential_kernel(const uint32_t* __restrict__ w,
...
void QMatrix::make_sequential(const uint32_t* cpu_g_idx) {
  ...
  cudaMemcpyAsync(cuda_q_perm,
                  cpu_x_map16,
                  height * sizeof(uint16_t),
                  cudaMemcpyHostToDevice);
  cudaMemcpyAsync(cuda_q_invperm,
                  cpu_x_map_inv16,
                  height * sizeof(uint16_t),
                  cudaMemcpyHostToDevice);
  ...
  make_sequential_kernel<<<gridDim,
                           blockDim,
                           0,
                           at::cuda::getCurrentCUDAStream()>>>(
      cuda_q_weight, cuda_new_qweight, cuda_q_perm, height / 8, width);

  // Replace qweights

  cudaMemcpyAsync(cuda_q_weight,
                  cuda_new_qweight,
