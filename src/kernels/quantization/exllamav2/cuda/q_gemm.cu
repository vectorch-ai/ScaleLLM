...
__compat_hipblasHgemm(hipblasHandle_t handle,
...
void gemm_half_q_half_cuda_part(const half* a,
                                QMatrix* b,
                                half* c,
                                int size_m,
                                int size_n,
  ...
  if (!b->is_gptq) {
    ...
    fp_gemm_half_q_half_kernel kernel =
        pick_gemm_half_q_half_kernel(true, m_count);

    kernel<<<gridDim, blockDim, 0, stream>>>(a,
                                             b->cuda_q_weight,
                                             b->cuda_q_scale,
                                             b->cuda_q_scale_max,
                                             c,
  ...
  if (size_m > MAX_Q_GEMM_ROWS && !force_cuda) {
    ...
    if (max_chunks) {
      gemm_half_q_half_cuda_part(
          a, b, c, last_chunk, size_n, size_k, BLOCK_M_SIZE_MAX, clear);
    }
...
__global__ void clear_kernel(half* __restrict__ c,
                             const int size_m,
                             const int size_n) {
  int m = blockIdx.y;
...
void clear_tensor_cuda(half* c, int size_m, int size_n) {
  return;
  dim3 blockDim, gridDim;
