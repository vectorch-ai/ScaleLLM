...
__forceinline__ __device__ void dequant_4bit_8_gptq(const uint32_t q_0,
  ...
  if (scaled) {
    dq[0] = __hfma2(q0.as_half2,
                    y1y16[0],
                    z1z16[0]);  // half2( q[0] * s - z * s, q[1] * s - z * s)
    dq[1] = __hfma2(q1.as_half2,
                    y1y16[1],
    ...
    dq[1] = __hfma2(
        q1.as_half2, y1y16[1], z1z16[1]);    // half2( q[2] - z, q[3] - z )
    dq[2] = __hadd2(q2.as_half2, z1z16[0]);  // half2( q[4] - z, q[5] - z )
    dq[3] = __hfma2(
        q3.as_half2, y1y16[1], z1z16[1]);  // half2( q[6] - z, q[7] - z )
  }
}
...
__forceinline__ __device__ void dequant_4bit_8_gptq(const uint32_t q_0,
  ...
  uint32_t qa = q_0;
  for (int i = 0; i < 4; i++) {
    half d0 = __int2half_rn(qa & 0x0f);
    qa >>= 4;
    half d1 = __int2half_rn(qa & 0x0f);
    qa >>= 4;
