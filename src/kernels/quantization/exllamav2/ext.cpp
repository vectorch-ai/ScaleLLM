...
uintptr_t make_q_matrix(torch::Tensor q_weight,
                        torch::Tensor q_perm,
  ...
  TORCH_CHECK(temp_dq.size(0) >= width * height,
...
void gemm_half_q_half(torch::Tensor a,
                      uintptr_t b,
                      torch::Tensor c,
                      bool force_cuda) {
  ...
  gemm_half_q_half_cuda(at::cuda::getCurrentCUDABlasHandle(),
