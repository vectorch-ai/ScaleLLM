...
#define CALL_IF(                                                      \
    ...
    cudaFuncSetAttribute(Marlin<THREADS,                              \
                                THREAD_M_BLOCKS,                      \
                                THREAD_N_BLOCKS,                      \
                                THREAD_K_BLOCKS,                      \
                                STAGES,                               \
...
void marlin_dense(const void* A,
                  const void* B,
                  void* C,
                  const void* s,
                  int prob_m,
  ...
  CHECK(prob_n % thread_n == 0 && prob_k % thread_k == 0 &&
  ...
  for (int i = 0; i < tot_m_blocks; i += 4) {
    ...
    CALL_IF(4, 16, 4, 8)
      ...
      LOG(FATAL) << "No kernel implementation for thread_k=" << thread_k
