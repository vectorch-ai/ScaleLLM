# adapted from https://github.com/triton-lang/triton/blob/main/python/triton/tools/compile.py
import binascii
import hashlib
...
if __name__ == "__main__":
    ...
    parser.add_argument("--target", "-t", type=str, default=None, help="Target architecture")
    ...
    # convert cuda:80 to GPUTarget
    def get_target(target):
        if target is None:
            return None
        parts = target.split(":")
        assert len(parts) >= 2, f"Invalid target string: {target}"
        backend = parts[0]
        arch = int(parts[1])
        warp_size = int(parts[2]) if len(parts) > 2 else 32
        if backend == "cuda":
            return GPUTarget(backend="cuda", arch=arch, warp_size=warp_size)
        elif backend == "hip":
            return GPUTarget(backend="hip", arch=arch, warp_size=warp_size)
        else:
          raise ValueError(f"Unknown target: {target}")
    
    ccinfo = triton.compile(src, target=get_target(args.target), options=opts)
    arg_names = []
    arg_types = []
    arg_names_not_1 = []
    arg_types_not_1 = []
    ...
    # write ptx and sass for testing
    with out_path.with_suffix(f".{sig_hash}_{suffix}.ptx").open("w") as fp:
        fp.write(ccinfo.asm["ptx"])
    with out_path.with_suffix(f".{sig_hash}_{suffix}.sass").open("w") as fp:
        fp.write(ccinfo.asm["sass"])