...
*/
__global__ void gemv_kernel_g64(const float4* _inputs,
  ...
  // TODO (Haotian): zeros_w is incorrect, after fixing we got misaligned
  // address
  const int zeros_w =
      make_divisible(make_divisible(IC / group_size, PACK_FACTOR), 2) * 2;
  ...
  for (int packed_group_idx = 0; packed_group_idx < num_groups_packed / 2;
    ...
    uint64_t packed_zeros = *reinterpret_cast<const uint64_t*>(
        zeros + oc_idx * zeros_w + packed_group_idx * 2);
    ...
    *((float4*)(packed_weights)) =
    ...
    float scaling_factor =
        __half2float(scaling_factors[oc_idx * sf_w + packed_group_idx * 16 +
                                     (threadIdx.x / 2)]);
    float current_zeros =
        (float)((packed_zeros >> (threadIdx.x / 2 * 4)) & 0xF);
...
#pragma unroll
        for (int ic_1 = 0; ic_1 < PACK_FACTOR; ic_1++) {
          ...
          // threadIdx.y == 0 && ic_0 == 0 && ic_1 == 0 && packed_group_idx ==
          // 0) printf("%f %f %f %f %X %X\n", dequantized_weight,
          // current_single_weight_fp, scaling_factor, current_zeros,
          // current_packed_weight, packed_zeros);
...
Notes:
  ...
  the second dimension is rounded up to a multiple of PACK_FACTOR.
*/
__global__ void gemv_kernel_g128(const float4* _inputs,
                                 const uint32_t* weight,
                                 const uint32_t* zeros,
                                 const half* scaling_factors,
                                 half* _outputs,
  ...
  for (int packed_group_idx = 0; packed_group_idx < num_groups_packed;
    ...
    *((float4*)(packed_weights)) =
    ...
    float scaling_factor =
        __half2float(scaling_factors[oc_idx * sf_w + packed_group_idx * 8 +
                                     (threadIdx.x / 4)]);
    float current_zeros =
        (float)((packed_zeros >> (threadIdx.x / 4 * 4)) & 0xF);
    int inputs_ptr_delta = packed_group_idx * WARP_SIZE * 4 + threadIdx.x * 4;
    const float4* inputs_ptr = inputs + inputs_ptr_delta;
// multiply 32 weights with 32 inputs
#pragma unroll
...
*/
torch::Tensor gemv_forward_cuda(torch::Tensor _in_feats,
