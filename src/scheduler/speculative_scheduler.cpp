...
void SpeculativeScheduler::step(const absl::Duration& timeout) {
  ...
  auto ssm_output_parameters = ssm_engine_->execute_model(sequences_batch_);
  // TODO: execute on validation step in llm
  auto llm_output_parameters = llm_engine_->execute_model(sequences_batch_);
...
void SpeculativeScheduler::on_request_finish(Request* request) {
  // release all blocks for the finished request
  llm_block_manager_->release_slots_for_request(request);
  // take over the ownership of the request
...
void SpeculativeScheduler::on_sequence_stream(Sequence* seq) {
...
void SpeculativeScheduler::build_sequence_batch() {
  ...
  while (!priority_queue_.empty()) {
    ...
    sequence_candidates.reserve(candidate->sequences.size());
    for (Sequence& sequence : candidate->sequences) {
      if (sequence.is_finished()) {
        // skip finished sequence.
        continue;
      }
      if (llm_block_manager_->allocate_slots_for_sequence(&sequence)) {
        sequence_candidates.push_back(&sequence);
      } else {
    ...
    if (has_enough_slots) {
      ...
      sequences_batch_.insert(sequences_batch_.end(),
                              sequence_candidates.begin(),
                              sequence_candidates.end());
    ...
    if (!preemptable_candidates_.empty()) {
      ...
      if (request_to_preempt != candidate) {
        llm_block_manager_->release_slots_for_request(request_to_preempt);
      }
      continue;
    }
