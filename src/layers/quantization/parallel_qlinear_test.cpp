...
namespace llm {
namespace {
// TODO: create a quantized state dict for testing
// std::shared_ptr<StateDict> create_quant_state_dict(int64_t in_features,
...
//   auto weight = torch::rand({out_features, in_features}, options);
...
TEST(QlinearTest, ColumnParallelQLinear) {
  ...
  ColumnParallelQLinearImpl qlinear(in_features,
                                    out_features,
                                    /*bias=*/false,
                                    quant_args,
                                    /*qweight_pack_dim=*/0,
...
TEST(QlinearTest, RowParallelQuantLinear) {
  ...
  quant_args.group_size(128);
  const auto options = torch::dtype(torch::kHalf).device(torch::kCUDA);
  RowParallelQLinearImpl qlinear(in_features,
                                 out_features,
                                 /*bias=*/false,
                                 quant_args,
                                 /*qweight_pack_dim=*/0,
