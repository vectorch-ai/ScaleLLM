...
void VLMWorker::capture_cuda_graph(uint32_t batch_size) {
  CHECK(model_ != nullptr) << "Model is not initialized.";
...
void VLMWorker::process_group_test() {
...
std::optional<ModelOutput> VLMWorker::execute_model(const ModelInput& inputs) {
  torch::DeviceGuard device_guard(device_);
...
folly::SemiFuture<std::optional<ModelOutput>> VLMWorker::execute_model_async(
...
folly::SemiFuture<folly::Unit> VLMWorker::process_group_test_async() {
...
folly::SemiFuture<folly::Unit> VLMWorker::capture_cuda_graph_async(
    uint32_t batch_size) {
  folly::Promise<folly::Unit> promise;
  auto future = promise.getSemiFuture();
  threadpool_.schedule(
